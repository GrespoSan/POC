...
uploaded_file = st.sidebar.file_uploader("Carica lista ticker (.txt)", type=["txt"])
...
lookback = st.sidebar.number_input("Periodo storico", min_value=100, max_value=2000, value=500)
swing_window = st.sidebar.slider("Pivot Window", min_value=2, max_value=20, value=5)
atr_period = st.sidebar.slider("ATR Period", min_value=5, max_value=50, value=14)
min_atr_ratio = st.sidebar.slider("Min ATR Ratio", min_value=0.5, max_value=5.0, value=1.5, step=0.1)
poc_tolerance = st.sidebar.slider("Distanza massima dal POC (%)", min_value=0.5, max_value=10.0, value=3.0, step=0.5)
ema_period = st.sidebar.number_input("EMA Period", min_value=20, max_value=300, value=200)
...
if st.button("🚀 Avvia Screener"):
    ...
    with st.spinner("Analisi titoli in corso..."):
        # CHIAMATA ESPLICITA PER NOMINATIVO (KEYWORD ARGUMENTS)
...
if "results" in st.session_state:
    ...
    if results.empty:
        ...
        selected = st.selectbox("Seleziona titolo", results["Ticker"].tolist())
        if selected:
            st.divider()
            st.subheader(f"Analisi {selected}")
            df = load_ticker(selected, lookback)
            swing = detect_macro_swing(df, window=swing_window, atr_period=atr_period, min_atr_ratio=min_atr_ratio)
            ...
            if swing is None:
                ...
                col2.metric("Swing %", f"{swing.move_pct:.2f}%")
                col3.metric("POC", f"{profile.poc:.2f}")
                col4.metric("Score", f"{swing.score:.1f}")